import type { IScreenshot } from '@interfaces';

export const screenshots: IScreenshot[] = [
  {
    text: 'Login screen with authentication form and branding',
    src: '/img/screenshots/demo-1-login-screen.avif',
  },
  {
    text: 'Initial setup step 1 – introduction or welcome configuration screen',
    src: '/img/screenshots/demo-2-setup-1.avif',
  },
  {
    text: 'Initial setup step 2 – server or connection configuration',
    src: '/img/screenshots/demo-2-setup-2.avif',
  },
  {
    text: 'Initial setup step 3 – account or authentication setup',
    src: '/img/screenshots/demo-2-setup-3.avif',
  },
  {
    text: 'Initial setup step 4 – folder or storage configuration',
    src: '/img/screenshots/demo-2-setup-4.avif',
  },
  {
    text: 'Initial setup step 5 – integration or service configuration',
    src: '/img/screenshots/demo-2-setup-5.avif',
  },
  {
    text: 'Initial setup step 6 – final review and confirmation',
    src: '/img/screenshots/demo-2-setup-6.avif',
  },
  {
    text: 'Movies library – poster table view with grid layout',
    src: '/img/screenshots/demo-3-movies-library-1-poster-table.avif',
  },
  {
    text: 'Movies library – filter menu with filtering options',
    src: '/img/screenshots/demo-3-movies-library-2-filter-menu.avif',
  },
  {
    text: 'Movies library – sorting menu with sort options',
    src: '/img/screenshots/demo-3-movies-library-2-sort-menu.avif',
  },
  {
    text: 'Movies library – full detailed view of selected movie',
    src: '/img/screenshots/demo-3-movies-library-2-full-view.avif',
  },
  {
    text: 'TV show library – download confirmation dialog',
    src: '/img/screenshots/demo-4-tv-show-library-1-download-confirmation.avif',
  },
  {
    text: 'Movies library – table view with list layout',
    src: '/img/screenshots/demo-3-movies-library-2-table-view.avif',
  },
  {
    text: 'TV show library – detail view with episode information',
    src: '/img/screenshots/demo-4-tv-show-library-1-detail-view.avif',
  },
  {
    text: 'Movies library – refresh progress and status overview',
    src: '/img/screenshots/demo-5-movies-library-refresh.avif',
  },
  {
    text: 'TV show library – refresh progress and status overview',
    src: '/img/screenshots/demo-6-tv-show-library-refresh.avif',
  },
  {
    text: 'Download overview – All downloads will show here with progress and status',
    src: '/img/screenshots/demo-7-download-screen.avif',
  },
  {
    text: 'Settings – account overview and management screen',
    src: '/img/screenshots/demo-5-settings-account-overview.avif',
  },
  {
    text: 'Settings – create or add new account screen',
    src: '/img/screenshots/demo-5-settings-new-account.avif',
  },
  {
    text: 'Settings – advanced configuration overview',
    src: '/img/screenshots/demo-5-settings-advanced-overview.avif',
  },
  {
    text: 'Settings – folder configuration overview',
    src: '/img/screenshots/demo-5-settings-folder-overview.avif',
  },
  {
    text: 'Settings – integration configuration overview',
    src: '/img/screenshots/demo-5-settings-integration-overview.avif',
  },
  {
    text: 'Settings – user interface customization overview',
    src: '/img/screenshots/demo-5-settings-ui-overview.avif',
  },
  {
    text: 'Settings – language select and other general settings overview',
    src: '/img/screenshots/demo-5-settings-ui-language-select.avif',
  },
  {
    text: 'Unraid Community Applications',
    src: '/img/screenshots/demo-10-unraid-ca.avif',
  },
];
